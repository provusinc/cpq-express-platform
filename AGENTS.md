...
- **Settings** has one route per tab under `settings/` (`company`, `quoting`, `labels`), listed in `components/settings/tabs.ts`; the Admin check is in `settings/layout.tsx`. Add a tab (Documents, #21) with an entry there and its page.
