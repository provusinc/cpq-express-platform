...
/**
 * Clone of niko-table's `filters/table-view-dnd-menu.tsx` (registry item
 * `@niko-table/data-table-view-dnd-menu`), carrying the app's Columns popover design (a
 * "n of m shown" header, checkbox rows, a padded Reset footer). The popover's
 * inside can't be styled from outside, so per the UI component policy this is
 * the one sanctioned clone; keep it in step with upstream fixes.
 */
...
import { formatLabel } from "@workspace/ui/components/niko-table/lib/format"
...
export interface ColumnsViewDndMenuProps<TData extends RowData> {
...
export function ColumnsViewDndMenu<TData extends RowData>({
  table,
...
ColumnsViewDndMenu.displayName = "ColumnsViewDndMenu"
