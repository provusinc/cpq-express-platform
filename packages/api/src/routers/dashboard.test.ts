...
describe("dashboard.overview", () => {
  ...
  it("sums the open pipeline, leaving out Won and Lost", () =>
    withTestDb(async (db) => {
      const { caller, quote, approver } = await setup(db)
      await quote("draft", { total: "100" })
      const rejected = await quote("draft", { total: "50.5" })
      await createApprovalStep(db, rejected, {
        action: "reject",
        actor: approver,
      })
      await quote("with_customer", { total: "25" })
      await quote("won", { total: "300" })
      await quote("lost", { total: "70" })
  ...
  it("queues what waits for an Approver, and a Member's own pending Quotes", () =>
    withTestDb(async (db) => {
      ...
      const recent = await quote("in_approval", { name: "Recent" })
      const old = await quote("in_approval", { name: "Old" })
      const own = await quote("in_approval", {
        name: "Approver's own",
        owner: approver,
      })
  ...
  it("counts only this Organization's Quotes, and refuses non-members", () =>
    withTestDb(async (db) => {
      ...
      for (const stage of ["in_approval", "draft", "won"] as const) {
...
describe("dashboard.valueOverTime", () => {
  ...
  it("sums created Quotes by UTC day, leaving out older ones", () =>
    withTestDb(async (db) => {
      ...
      await quote("won", { total: "20", createdAt: new Date() })
      ...
      // Created isn't Won: that needs its Approval Step.
      expect(buckets.every((b) => b.won.count === 0)).toBe(true)
    }))

  it("dates Won by its Approval Step, in weeks", () =>
    withTestDb(async (db) => {
      const { user, caller, quote } = await setup(db)
      const won = await quote("won", {
        total: "300",
        createdAt: daysAgo(200),
      })
      const other = await quote("won", {
        total: "50.5",
        createdAt: daysAgo(20),
      })
      const refused = await quote("draft", { total: "999" })
      await createApprovalStep(db, won, {
        action: "customer_approved",
        actor: user,
      })
      await createApprovalStep(db, other, {
        action: "customer_approved",
        actor: user,
      })
      await createApprovalStep(db, refused, {
      ...
      expect(result.buckets.at(-1)!.won).toEqual({
        count: 2,
        value: "350.5000",
      })
      ...
      // `won` was created before the range; `other` and `refused` in it.
